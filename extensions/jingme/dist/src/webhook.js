...
async function routeMessage(message, account) {
    ...
    try {
        ...
        const dispatcher = {
            async sendBlockReply(block) {
                // Handle text replies
                const replyText = (block.markdown || block.text || '');
                if (!replyText.trim())
                    return;
...
function createRequestHandler(account) {
    ...
    return async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                if (bodyObj.encrypt &&
                    ...
                    try {
                        ...
                        api.logger.error(`[jingme] Failed to decrypt message: ${decryptError}`);
                ...
                api.logger.error(`[jingme] Failed to parse request body: ${parseError}`);
            ...
            if (bodyObj.challenge) {
                api.logger.info(`[jingme] Verification request detected, challenge: ${bodyObj.challenge}`);
            ...
            try {
                if (bodyObj.eventType === 'chat_message') {
                    ...
                    api.logger.info(`[jingme] 暂时不支持处理该类型的消息${bodyObj.eventType}`);
                    return sendResponse(res, 200, resultInfo);
                }
            }
            catch (routeError) {
                const routeErrorMsg = routeError instanceof Error ? routeError.message : String(routeError);
...
export function startWebhookServer(account, abortSignal) {
    const api = getJingmeRuntime();
    ...
    const createServer = () => {
        ...
        server.on('error', (error) => {
            ...
            if (restartCount < MAX_RESTART_ATTEMPTS) {
                restartCount++;
                api.logger.info(`[jingme] Restarting webhook server (attempt ${restartCount}/${MAX_RESTART_ATTEMPTS})`);
                console.warn(`[jingme] Restarting webhook server (attempt ${restartCount}/${MAX_RESTART_ATTEMPTS})`);
                restartTimeout = setTimeout(() => {
                    webhookServer.server = createServer();
                    webhookServer.server.listen(port, () => {
                        api.logger.info(`[jingme] Webhook server restarted on port ${port}`);
                    });
                }, RESTART_DELAY_MS);
            }
            else {
                console.error(`[jingme] Max restart attempts (${MAX_RESTART_ATTEMPTS}) reached, giving up`);
            }
        });
        return server;
    };
